...
const mediaItems = [
  ...
  {
    type: "image",
    src: "/images/battery-1024x683.jpg",
    title: "High Voltage Stack",
    category: "Enterprise",
    alt: "High voltage battery stack",
  },
  {
    type: "image",
...
export function BatteryShowcase() {
    ...
    const interval = setInterval(() => {
      if (scrollContainerRef.current) {
        ...
        if (scrollLeft + clientWidth >= scrollWidth - 10) {
          ...
          scrollContainerRef.current.scrollBy({ left: Math.min(500, clientWidth * 0.7), behavior: 'smooth' });
        }
      }
    }, 3500);
  ...
  const scrollLeft = () => {
    if (scrollContainerRef.current) {
      ...
      scrollContainerRef.current.scrollBy({ left: -Math.min(500, containerWidth * 0.7), behavior: 'smooth' });
    }
  };
      ...
      <section className="relative w-full overflow-hidden bg-zinc-950 py-24 sm:py-32">
          ...
          <ScrollReveal animation="fade-up">
            <div className="text-center">
              ...
              <p className="mx-auto max-w-2xl text-lg text-zinc-400">
                Explore our high-performance battery packs and seamless inverter installations designed to keep you powered 24/7.
              </p>
            </div>
          </ScrollReveal>
        </div>
        ...
        >
          ...
          >
            ...
            {[...mediaItems, ...mediaItems, ...mediaItems, ...mediaItems].map((item, index) => (
              <div 
                key={`${item.src}-${index}`}
