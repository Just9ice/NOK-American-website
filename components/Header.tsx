...
export default function Header() {
  return (
    <header className="fixed w-full top-0 z-50 pt-4">
      <nav
        className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 rounded-3xl border border-white/10 bg-black/25 backdrop-blur-xl shadow-[0_8ox_rgba(0,0,0,0.15)]"
        aria-label="Top"
      >
        ...
        <div className="flex flex-wrap justify-center space-x-6 py-4 md:hidden">
          {navigation.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className="text-base font-medium text-gray-700 hover:text-[#17365d]"
            >
              {link.name}
            </Link>
          ))}
